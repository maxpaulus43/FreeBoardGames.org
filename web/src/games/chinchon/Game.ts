...
import { calculatePointsForHand, canMeldWithCard, cardCompareFn, removeJokersFromCards } from './MeldLogic';
import {
  CardSuit,
  ChinchonCard,
  ChinchonCtx,
...
const discardCard: Move<ChinchonGameState> = (G, ctx, theCard: ChinchonCard) => {
  const p = G.players[ctx.currentPlayer];
  const hand = p.hand;
...
const meldHandWithCard: Move<ChinchonGameState> = (G, ctx, meldCard: ChinchonCard) => {
...
function scoreAndEliminatePlayers(G: ChinchonGameState, ctx: ChinchonCtx) {
  ...
  if (G.roundEndState[ctx.currentPlayer].points === -Infinity) {
    Object.entries(G.players)
      .filter(([pId]) => pId !== ctx.currentPlayer)
      .forEach(([pId, player]) => {
