...
export function canMeldWithCard(cards: ChinchonCard[], meldCard: ChinchonCard): boolean {
  return getMeldablePermutationsForHand(cards, meldCard).length > 0;
}
function getMeldablePermutationsForHand(cards: ChinchonCard[], meldCard: ChinchonCard): ChinchonCard[][] {
...
export function calculatePointsForHand(G: ChinchonGameState, hand: ChinchonCard[]): [number, ChinchonCard[]] {
...
function canMeldPermutation(cards: ChinchonCard[]): boolean {
  return is7CardRun(cards) || isMeldable6CardRun(cards) || is4_3Combo(cards) || isMeldable3_3Combo(cards);
}
